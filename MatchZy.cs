...
namespace MatchZy
{
    ...
    public partial class MatchZy : BasePlugin
    {
        ...
        public override string ModuleVersion => "0.10.2";
        ...
        public string chatPrefix = $"[{ChatColors.DarkBlue}IGL-MatchZy{ChatColors.Default}]";
        ...
        public override void Load(bool hotReload)
        {
            ...
            RegisterEventHandler<EventPlayerHurt>(
                (@event, info) =>
                {
                    ...
                    if (
                        !attacker!.IsValid
                        || attacker.IsBot && !(@event.DmgHealth > 0 || @event.DmgArmor > 0)
                    )
                        return HookResult.Continue;
                    ...
                    if (message.StartsWith(".asay"))
                    {
                        if (IsPlayerAdmin(player, "css_asay", "@css/chat"))
                        {
                            ...
                            else
                            {
                                ...
                                ReplyToUserCommand(
                                    player,
                                    Localizer["matchzy.cc.usage", ".asay <message>"]
                                );
                            }
                        }
                        else
