...
namespace MatchZy
{
    ...
    public partial class MatchZy
    {
        ...
        public void StartPracticeMode()
        {
            ...
            {
                Log(
                    $"[StartWarmup] Starting Practice Mode! Executing Practice CFG from {practiceCfgPath}"
                );
        ...
        private void HandleSaveNadeCommand(CCSPlayerController? player, string saveNadeName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(saveNadeName))
            {
                ...
                try
                {
                    ...
                    PrintToPlayerChat(
                        player,
                        Localizer["matchzy.pm.lineupsavedsucces", lineupName]
                    );
                    PrintToAllChat(
                        Localizer[
                ...
                try
                {
                    ...
                    )
                    {
                        ...
                        if (lineupInfo.ContainsKey("Map") && lineupInfo["Map"] == Server.MapName)
                        {
                            ...
                            ReplyToUserCommand(
                                player,
                                Localizer["matchzy.pm.lineupdeletesuccess", saveNadeName]
                            );
                        }
                        else
                        {
                            ...
                            ReplyToUserCommand(
                                player,
                                Localizer["matchzy.pm.nadenotfoundonmap", saveNadeName]
                            );
                        }
                    }
                    else
                    {
                        ...
                        ReplyToUserCommand(
                            player,
                            Localizer["matchzy.pm.lineupnotfound", saveNadeName]
                        );
                    }
                }
                catch (JsonException ex)
                    ...
                    if (parts.Length == 7)
                    {
                        ...
                        )
                        {
                            ...
                            )
                            {
                                ...
                                ReplyToUserCommand(
                                    player,
                                    Localizer["matchzy.pm.lineupalreadyexists", lineupName]
                                );
                                return;
                            }
                        }
            ...
            if (!string.IsNullOrWhiteSpace(loadNadeName))
            {
                ...
                try
                {
                        ...
                        if (savedNadesDict.ContainsKey(currentSteamID))
                        {
                            ...
                            if (savedNadesDict[currentSteamID].ContainsKey(nearestName))
                            {
                                ...
                                )
                                {
                                    ...
                                    ReplyToUserCommand(
                                        player,
                                        Localizer["matchzy.pm.lineuploadedsuccess", nearestName]
                                    );
                                    ...
                                    if (!string.IsNullOrWhiteSpace(lineupDesc))
                                    {
                                        ...
                                        ReplyToUserCommand(
                                            player,
                                            Localizer["matchzy.pm.lineupdesc", lineupDesc]
                                        );
                                    }
                                ...
                                else
                                {
                                    ...
                                    ReplyToUserCommand(
                                        player,
                                        Localizer["matchzy.pm.nadenotfoundonmap", nearestName]
                                    );
                    ...
                    if (!lineupFound && !lineupOnWrongMap)
                    {
                        ...
                        ReplyToUserCommand(
                            player,
                            Localizer["matchzy.pm.nadenotfound", loadNadeName]
                        );
                    }
                }
                catch (JsonException ex)
        ...
        public void TemporarilyDisableCollisions(CCSPlayerController p1, CCSPlayerController p2)
        {
            ...
            collisionGroupTimer = AddTimer(
                ...
                () =>
                {
                    ...
                    {
                        Log(
                            $"player handle invalid p1p {p1p.Value.IsValid} p2p {p2p.Value.IsValid}"
                        );
                        collisionGroupTimer?.Kill();
                        return;
                    }
            ...
            if (isPractice && player!.IsValid && player.IsBot && player.UserId.HasValue)
            {
                if (pracUsedBots.ContainsKey(player.UserId.Value))
                {
                    if (pracUsedBots[player.UserId.Value]["position"] is Position botPosition)
                    {
                        ...
                        CCSPlayerController? botOwner = (CCSPlayerController)
                            pracUsedBots[player.UserId.Value]["owner"];
                        if (
                            botOwner != null
                            && botOwner.IsValid
        ...
        public void RemoveGrenadeEntities()
        {
            if (!isPractice)
                return;
            var smokes = Utilities.FindAllEntitiesByDesignerName<CSmokeGrenadeProjectile>(
            ...
            var mollys = Utilities.FindAllEntitiesByDesignerName<CSmokeGrenadeProjectile>(
                "molotov_projectile"
            );
            ...
            var inferno = Utilities.FindAllEntitiesByDesignerName<CSmokeGrenadeProjectile>(
                "inferno"
            );
        ...
        public void ExecDryRunCFG()
        {
            ...
            {
                Log(
                    $"[ExecDryRunCFG] Starting Dryrun! Dryrun CFG not found in {absolutePath}, using default CFG!"
                );
                Server.ExecuteCommand(
        ...
        public bool IsValidPositionForLastGrenade(CCSPlayerController player, int position)
        {
            ...
            if (lastGrenadesData[userId].Count < position)
            {
                ...
                PrintToPlayerChat(
                    player,
                    Localizer["matchzy.pm.grenadehistory", $"{lastGrenadesData[userId].Count}"]
                );
                return false;
            }
        ...
        public void HandleDelayCommand(CCSPlayerController player, string delay)
        {
            ...
            if (string.IsNullOrWhiteSpace(delay))
            {
                ...
                ReplyToUserCommand(
                    player,
                    Localizer["matchzy.cc.usage", $"!delay <delay_in_seconds>"]
                );
                return;
            }
        ...
        public void OnSavePosCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            Log(
                $"[SavePos] Saved position for UserID {userId}, Position: {position}, Angle: {angle}!"
            );
        ...
        public void OnLoadPosCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            Log(
                $"[LoadPos] LoadPos position for UserID {userId}, Position: {playerLocationData.Position}, Angles: {playerLocationData.Angle}!"
            );
